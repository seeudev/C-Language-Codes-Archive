	...
	do{
		...
		switch(inp){
			...
			case 'A':
				...
				gotoxy(25,6); printf("Enter Array Size: "); 
				if(scanf("\n%d",&N)==1){
					if(N>50){
				...
				initializeArr(&ia[0],&N);
					...
					N=0;
					gotoxy(25,6); printf("Invalid INPUT!! Try Again. . ."); 
					gotoxy(22,7); printf("Array Corrupted. . . Please Initialize Again."); 
					gotoxy(18,9); printf("Press Any Key to Continue to Array Initialization. . .");
					inp=0;
					getch(); break;
				}
			...
			case 'C':
				...
				if(cFlag==1){
					...
					gotoxy(25,5); printf("Insert at Array[?]: "); 
					if(scanf("%d",&K)==1){
						if(K<N&&K>=0){
						N++;
			...
			case 'E':
				...
				if(cFlag==1){
					...
					gotoxy(1,24); printf("Press any key to continue. . ."); getch();
				}else{
						...
						int loc=locate(&ia[0],&N,ITEM);	
							cFlag=sameChecker(&ia[0],&N,ITEM);
								if(cFlag==1){
									...
									gotoxy(20,11); printf("Press any key to view PrintArray. . ."); getch();
									clrscr(); inputPage(); printArray(&ia[0],&N); 
									gotoxy(20,(loc+5));printf ("Located  %c",175);
									gotoxy(1,24); printf("Press any key to continue. . ."); getch();
								}else{
									gotoxy(25,6); printf("ARRAY INTEGER NOT FOUND. . . Try again."); getch();
									break;
