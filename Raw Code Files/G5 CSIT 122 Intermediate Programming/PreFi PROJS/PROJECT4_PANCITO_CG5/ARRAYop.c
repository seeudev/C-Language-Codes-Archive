...
void initializeArr(int la[0],int *n){
	int i=0,j,l=*n,inp;
		for(i=0;i<l;i++){
			...
			gotoxy(25,8);printf("Enter Data in Array[%d]: ",i); 
			if(scanf("\n%d",&inp)==1){
				la[i]=inp;
			}else{
				gotoxy(20,10); printf("Input INVALID!! Try again. . ."); 
				gotoxy(22,11); printf("Array Corrupted. . . Please Initialize Again."); 
				gotoxy(18,13); printf("Press Any Key to Continue to Array Initialization. . .");
				la[i]=0;
				i--;
				la[i]=0;
				getch(); 
